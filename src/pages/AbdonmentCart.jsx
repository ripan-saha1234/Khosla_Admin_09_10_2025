...
function AbdonmentCart() {
  ...
  useEffect(() => {
    const fetchingOrders = async () => {
      ...
      // Filter only "Order initiated" status orders
      const initiatedOrders = parsedOrders.filter(order => 
        order.order_status && order.order_status.toLowerCase() === "order initiated"
      );
      setOrders(initiatedOrders);
  ...
  const handleStatusChange = async (orderId, phoneNo, newStatus) => {
    try {
      ...
      if (data.statusCode === 200 || data.statusCode === 201 || data.success === true || data.message === "Order status updated successfully") {
        ...
        // Filter only initiated orders after update
        const initiatedOrders = updatedOrders.filter(order => 
          order.order_status && order.order_status.toLowerCase() === "order initiated"
        );
        ...
        if (searchTerm === "") {
          ...
          const filtered = initiatedOrders.filter(order => {
            if (searchType === "phone_no") {
              return order.phone_no.toLowerCase().includes(searchTerm.toLowerCase());
            } else {
              return order.order_id.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      <div className="orders-page-order-container">
        {filteredOrders.length === 0 ? (
          <div style={{ padding: "20px", textAlign: "center" }}>
            <p>No orders with "Order initiated" status found.</p>
          </div>
        ) : (
          filteredOrders
            .slice()
